node {
    ...
    stage('Deploy') {
    	if (env.BRANCH_NAME=='ecs'){
    		sh 'docker run --rm -v ~/.aws:/root/.aws amazon/aws-cli:latest ecs --cluster myCluster2 --service book-service --deployment-configuration maximumPercentage=200,minimumHealthyPercentage=50 --force-new-deployment'
